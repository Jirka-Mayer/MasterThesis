...
parser.add_argument("--supervised_ratio", default=0.1, type=float, help="Ratio of labeled samples.")
...
def main(args):
    ...
    model = AaeModel(seed=args.seed, supervised_ratio=args.supervised_ratio)
    ...
    model.visualize_sample(-1, scatter_images[:20], scatter_labels[:20])
    ...
    model.fit(
        ...
        callbacks=[
            ...
            tf.keras.callbacks.LambdaCallback(
                on_epoch_end=lambda e, l: model.visualize_sample(
                    e, scatter_images[:20], scatter_labels[:20]
                )
            )
        ]
    )
